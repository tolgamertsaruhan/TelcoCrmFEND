...
export class SubmittedOrderComponent implements OnInit {
  ...
  // Additional resolved city/district
  districtName: string = '';
  cityName: string = '';
    ...
    this.orderService.getOrderById(this.orderId).subscribe({
      next: (res) => {
        setTimeout(() => {
          ...
          // serviceAddress → backend address ID
  ...
  resolveDistrictAndCity(districtId: string) {
